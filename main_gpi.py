...
# @ray.remote
# def worker(state_space_subset, control_space, noise_std_dev, T, progress_actor):
#     transition_matrix_subset = np.zeros((len(state_space_subset), len(control_space), 7, 2))
#     for index, state in enumerate(state_space_subset):
#         t = state[0]
#         E_next = compute_next_state(state, control_space)  # Ensure this function is defined
#         time_component = (t + 1) % T

#         for j, control in enumerate(control_space):
#             next_state = np.concatenate(([time_component], E_next[:, j]))
#             next_state_index = state_to_index(next_state)  # Ensure this function is defined
#             possible_next_states = compute_possible_next_states(next_state, noise_std_dev)  # Ensure this function is defined
#             probabilities = multivariate_normal.pdf(possible_next_states[:, 1:], mean=state_space[next_state_index, 1:], cov=np.diag(noise_std_dev))
#             normalized_probabilities = probabilities / probabilities.sum()
#             possible_indices = np.array([state_to_index(state) for state in possible_next_states])
#             transition_matrix_subset[index, j, :, 0] = possible_indices
#             transition_matrix_subset[index, j, :, 1] = normalized_probabilities

#         # Update progress after processing each state
#         ray.get(progress_actor.update.remote(1))
    
#     return transition_matrix_subset

# def parallel_computing(state_space, control_space, noise_std_dev, T):
#     ray.init()
#     num_workers = 8  # Adjust number of workers based on your system's CPU core count
#     chunk_size = len(state_space) // num_workers
#     total_progress = len(state_space)
#     progress_actor = ProgressActor.remote(total_progress)

#     jobs = [worker.remote(state_space[i:i + chunk_size], control_space, noise_std_dev, T, progress_actor) for i in range(0, len(state_space), chunk_size)]
#     transition_matrix = np.zeros((len(state_space), len(control_space), 7, 2))

#     results = ray.get(jobs)
#     ray.get(progress_actor.close.remote())  # Close the progress bar
#     ray.shutdown()

#     for i, result in enumerate(results):
#         start_index = i * chunk_size
#         end_index = start_index + result.shape[0]
#         transition_matrix[start_index:end_index] = result

#     np.save('transition_matrix.npy', transition_matrix)
    
#     return transition_matrix

# noise_std_dev = [0.04, 0.04, 0.004]
# transition_matrix = parallel_computing(state_space, control_space, noise_std_dev, T)
